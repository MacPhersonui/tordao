...
// File contracts/LunchDaoNFT.sol
...
contract LunchDaoNFT is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    mapping (address => bool) public isHoldNFT;
    mapping(address => uint256) public allowlist;


    event CreateLunchDaoNFT(address account, uint256 indexed id);

    constructor(uint256 _projectID,uint256 _nftTotalSupply, uint256 _startTime ,string memory _baseURI, address _iTicketBooth) ERC721("Lunch Dao NFT", "Lunch Dao NFT") {
        setBaseURI(_baseURI);
    ...
    function setNftTotalSupply(uint256 _nftTotalSupply) public onlyOwner {
        nftTotalSupply = _nftTotalSupply;
    }
    ...
    function mint() public saleIsOpen {
        _mintAnElement(msg.sender);
    }
    
    function _mintAnElement(address _owner) private {
        require(totalMint() <= nftTotalSupply, "The limit has been exceeded.");
        require(!isHoldNFT[_owner],"Already minted!");
        require(iTicketBooth.balanceOf(_owner, projectID) > 100000000000000000000000, "Less than 0.1 ETH.");
        uint id = _totalSupply();
        _tokenIdTracker.increment();
        _safeMint(_owner, id);
        isHoldNFT[_owner] = true;
        emit CreateLunchDaoNFT(_owner, id);
    }
    ...
    function allowlistMint() public saleIsOpen{
        require(totalMint() <= nftTotalSupply, "The limit has been exceeded.");
